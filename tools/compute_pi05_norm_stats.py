#!/usr/bin/env python3
"""Compute PI0.5 state/action statistics from LeRobot parquet data.

The script mirrors the important ordering used by OpenPI: robot values are
first represented in the model action space, selected absolute action
dimensions are converted to deltas from the current state, and statistics are
computed only then.  Normalization itself is deliberately not applied here.
...
class Profile:
    """Robot action-space fields required before PI0.5 normalization."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='First action offset. PI0.5/OpenPI uses 0. Default: 0.')
    parser.add_argument(
        '--exclude-terminal-padding',
        action='store_true',
        help='Exclude out-of-episode future actions instead of repeating the '
        'last action. PI0.5 real-robot configs normally repeat them.')
    ...
    parser.add_argument('--variable-name', default='_PI05_DATASET_STATS')
