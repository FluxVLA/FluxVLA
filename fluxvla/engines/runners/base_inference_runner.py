...
class BaseInferenceRunner:
    ...
    def _run_episode(self, default_instruction: str):
        ...
        while t < self.max_publish_step and not rospy.is_shutdown():
            ...
            for instruction in instructions:
                ...
                if self._poll_keyboard_stop():
                    self.reset_inference_history()
                    overwatch.info('Stopped after the current action chunk; '
    ...
    def _poll_keyboard_stop(self) -> bool:
        """Return True when the terminal requests an end after this chunk.

        A terminal is line-buffered, so type ``q`` and press Enter while a
        ...
        return command == self._STOP_COMMAND
