...
class LiberoParquetEvalDataset:
    ...
    def __call__(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.norm_stats is not None:
            if self.norm_stats_key is None:
                raise KeyError('norm_stats_key must be provided when '
                               'norm_stats are configured.')
            if self.norm_stats_key not in self.norm_stats:
                raise KeyError(
